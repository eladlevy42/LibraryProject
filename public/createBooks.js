...
//        if (allBooks.some(existingBook => validateInput(parsedBook, existingBook))){
...
//             if (allBooks.some(existingBook => validateInput(book, existingBook))){
//                 continue
//             }
...
//         if(validateInput(formDataObj, book)){
//             alert('book already exists');
//             return
//         }
//     }
...
//     try{
//     await axios.post(jsonServerUrl, formDataObj, {headers : {"Content-Type":"application/json"},})
...
//     }
// }
// function validateInput(input, book) {
//     const areAuthorsEqual = input.authors_name.every(author => book.authors_name.includes(author));
//     const areCategoriesEqual = input.categories.every(category => book.categories.includes(category));
//     return areAuthorsEqual &&
//            areCategoriesEqual &&
//            input.book_name === book.book_name &&
//            input.ISBN === book.ISBN &&
//            input.num_pages === book.num_pages;
// }
...
async function checkBookExists(bookName) {
    try {
        const response = await axios.get(`${jsonServerUrl}?book_name=${encodeURIComponent(bookName)}`);
...
document.getElementById('createBookByName').addEventListener('submit', function(event){
    event.preventDefault()
    createNewBookByName()
    document.getElementById('bookName').value = ''
})

document.getElementById('createBookByISBN').addEventListener('submit', function(event){
    event.preventDefault()
    createNewBookByISBN()
    document.getElementById('ISBN').value = ''
})
